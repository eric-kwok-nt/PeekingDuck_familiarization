...
class Node(AbstractNode):
    ...
    def __init__(self, config: Dict[str, Any] = None, **kwargs: Any) -> None:
        ...
        self.nose = np.array([-1.0, -1.0])
        self.left_eye = np.array([-1.0, -1.0])
        self.right_eye = np.array([-1.0, -1.0])
        self.lower_left_arm_vec = np.array([-1.0, -1.0])
        self.lower_right_arm_vec = np.array([-1.0, -1.0])
    ...
    def run(self, inputs: Dict[str, Any]) -> Dict[str, Any]:  # type: ignore
        ...
        for i, keypoint in enumerate(keypoints):
            ...
            if not (
                ...
                is_left_straight = self._is_straight_arm(
                    self.lower_left_arm_vec, left_elbow, left_shoulder
                )
                is_right_straight = self._is_straight_arm(
                    self.lower_right_arm_vec, right_elbow, right_shoulder
                )
                is_left_bent = self._is_bent_arm(
                    self.lower_left_arm_vec, left_elbow, left_shoulder
                )
                is_right_bent = self._is_bent_arm(
                    self.lower_right_arm_vec, right_elbow, right_shoulder
                )
                is_head_close_left_wrist = self._is_head_close_to_wrist_or_elbow(
                    left_wrist, left_elbow
                )
                is_head_close_right_wrist = self._is_head_close_to_wrist_or_elbow(
                ...
                if are_arms_parallel:
                    if is_left_straight and is_right_bent and is_head_close_right_wrist:
                        total_score = (
                            is_left_straight * self.score_weightage["straight_arm"]
                            + is_right_bent * self.score_weightage["bent_arm"]
                            + is_head_close_right_wrist
                            * self.score_weightage["head_wrist"]
                            + are_arms_parallel
                            * self.score_weightage["lower_arm_parallel"]
                        )

                        is_dab_list[i] = f"Dab Detected! Score: {total_score:.2f}"

                    elif (
                        is_right_straight and is_left_bent and is_head_close_left_wrist
                    ):
                        total_score = (
                            is_right_straight * self.score_weightage["straight_arm"]
                            + is_left_bent * self.score_weightage["bent_arm"]
                            + is_head_close_left_wrist
                            * self.score_weightage["head_wrist"]
                            + are_arms_parallel
                            * self.score_weightage["lower_arm_parallel"]
                        )
    ...
    def _is_head_close_to_wrist_or_elbow(
        ...
        for item in items_to_check:
            if not (item == [-1.0, -1.0]).all():
                item_wrist_dist = np.linalg.norm(item - wrist)
                item_elbow_dist = np.linalg.norm(item - elbow)
                dist = min(
                    [item_wrist_dist / lower_arm_len, item_elbow_dist / lower_arm_len]
                )
    ...
    def _are_lowerarms_parallel(self) -> Tuple[float, bool]:
        ...
        """
        parallel_score = cosine_similarity(
            [self.lower_left_arm_vec], [self.lower_right_arm_vec]
        ).ravel()[0]
    ...
    def _are_keypoints_present(
        ...
        """
        return (
            (left_wrist == [-1.0, -1.0]).all()
            or (right_wrist == [-1.0, -1.0]).all()
            or (left_elbow == [-1.0, -1.0]).all()
            or (right_elbow == [-1.0, -1.0]).all()
            or (left_shoulder == [-1.0, -1.0]).all()
            or (right_shoulder == [-1.0, -1.0]).all()
            or (
                (self.nose == [-1.0, -1.0]).all()
                and (self.left_eye == [-1.0, -1.0]).all()
                and (self.right_eye == [-1.0, -1.0]).all()
            )
        )
