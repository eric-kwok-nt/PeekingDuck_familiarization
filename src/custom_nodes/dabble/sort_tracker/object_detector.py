...
class Detector:
    ...
    def __init__(self, path_to_ckpt):
        ...
        self.detection_graph = tf_.Graph()

        with self.detection_graph.as_default():
            od_graph_def = tf_.GraphDef()
            with tf_.gfile.GFile(self.path_to_ckpt, 'rb') as fid:
                serialized_graph = fid.read()
                od_graph_def.ParseFromString(serialized_graph)
                tf_.import_graph_def(od_graph_def, name='')
        ...
        self.sess = tf_.Session(graph=self.detection_graph, config=config)
